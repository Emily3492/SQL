...
#day
index_name = pd.date_range(start='20161226',end='20170313')
...
df.head()
df.tail()
df.to_csv('day_liveness0310.csv', encoding='utf-8')

#week
#只需要改
index_name = pd.date_range('20170101',periods=12,freq='7D')#t+t1天的前一天的日期
df = pd.DataFrame(0,index=index_name,columns=columns_name)
t = int(time.mktime(time.strptime('2016-12-26 00:00:00',"%Y-%m-%d %H:%M:%S")))
t1 = 60*60*24*7
...
df.head()
df.tail()
df.to_csv('week_liveness0310.csv', encoding='utf-8')
...
t_start = int(time.mktime(time.strptime('20170313',"%Y%m%d")))
t_end = int(time.mktime(time.strptime('20170320',"%Y%m%d")))
owner1 = owner[owner.apply_timestamp < t_end]
log1 = log[(log.login_timestamp>=t_start) & (log.login_timestamp<t_end)]
...
for j in content2_list:
    ...
    df.tail()
    df.to_csv('dapartment_week0319.csv', encoding='gbk')
...
#固定时间
t = time.mktime(time.strptime('2017-03-20','%Y-%m-%d'))
t_list = {}
t_list['day'] = t-86400
